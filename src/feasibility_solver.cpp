...
bool feasibility_solver::solve(double t,
                               double t_lift,
                               bool dbl_supp,
                               ...
                               const double kappa)
{
  ...
  for(int j = 0; j <= N_ds_; j++)
  {
    ...
    if(doubleSupport_) { xTimings_(j) = exp(-eta_ * (t_ + alpha_j * (tds_ref - t_))); }
    else { xTimings_(j) = exp(-eta_ * t_); }
  }
  for(int i = 0; i <= N_steps; i++)
  {
    if(i != N_steps) { xStep_.segment(2 * i, 2) = optimalSteps_[i].translation().segment(0, 2); }
    if(i != 0)
    {
...
void feasibility_solver::update_x()
{
  ...
  for(size_t i = 1; i <= static_cast<size_t>(N_timings); i++)
  {
    ...
    for(size_t j = 0;
        j <= (i != static_cast<size_t>(N_timings) ? static_cast<size_t>(N_ds_) : static_cast<size_t>(N_tdsLast - 1));
        j++)
    {
      const double alpha_j = static_cast<double>(j) / static_cast<double>(N_ds_);
  ...
  {
    x_.segment(static_cast<Eigen::Index>(N_timings * (N_ds_ + 1) + N_tdsLast + 2 * i), 2) =
        optimalSteps_[i].translation().segment(0, 2);
  }
}

std::vector<Eigen::Vector3d> feasibility_solver::get_feasibility_region(const sva::PTransformd & X_0_supportFoot,
                                                                        const sva::PTransformd & X_0_swingFoot)
{
  ...
  Eigen::Vector4d feasibilityOffset =
      exp(eta_ * t_) * (A_f.block(0, 0, N_.rows(), N_variables - N_slack) * xTimings_ + b_f);
